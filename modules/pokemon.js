...
function send_pokemon(MAIN, internal_value, sighting, channel, time_now, main_area, sub_area, embed_area, server){
  ...
  MAIN.Static_Map_Tile(sighting.latitude,sighting.longitude,'pokemon','feed').then(async function(img_url){
    ...
    let pokemon_embed = new Discord.RichEmbed()
      ...
      .addField('**'+pokemon_name+'** ['+internal_value+'%] '+'(LVL '+sighting.pokemon_level+') '+'|A '+sighting.individual_attack+' |D '+sighting.individual_defense+' |S '+sighting.individual_stamina+'|', '\nDisappears: **~'+hide_time+'** (*~'+hide_minutes+' Mins*) [__**'+embed_area+'**__]', true)
	  .addField(weather_boost+' | CP '+sighting.cp+gender+' | ', height+' | '+weight, false)
      .addField(move_name_1+' '+move_type_1+' / '+move_name_2+' '+move_type_2, 'Type: '+pokemon_type, false)

      .addField(main_area+' | Directions:','[Google Maps](https://www.google.com/maps?q='+sighting.latitude+','+sighting.longitude+') | [Apple Maps](http://maps.apple.com/maps?daddr='+sighting.latitude+','+sighting.longitude+'&z=10&t=s&dirflg=w) | [Waze](https://waze.com/ul?ll='+sighting.latitude+','+sighting.longitude+'&navigate=yes)')
      .setImage(img_url);
...
async function send_without_iv(MAIN, sighting, channel, time_now, main_area, sub_area, embed_area, server){
  ...
  MAIN.Static_Map_Tile(sighting.latitude,sighting.longitude).then(async function(img_url){
    ...
    let pokemon_embed = new Discord.RichEmbed()
      .setColor('00ccff').setThumbnail(pokemon_url)
      .addField('A Wild **'+name+'** has Appeared!','Location:**'+embed_area+'**', false)
